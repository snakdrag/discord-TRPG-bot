...
class COUNT_MAIN(Cog_Extension):
    ...
    @app_commands.describe(text=constant.COUNT)
    ...
    async def _dice(
        ...
        if mode.startswith(Count_result.coc7e[:3]):
            try:
                value = Count_result.dnd_result(value)[1]
            except:
                return await interaction.response.send_message(
                    f"{constant.NOT} {constant.NUM}"
                )
    ...
    @_group.command(
        name=constant.LIST, description=f"{constant.CHOOSE}/{constant.RESORT}"
    )
    @app_commands.describe(
        mode=constant.MODE,
        list="使用空格分隔不同東西",
    ...
    async def _list_autocomplete_mode(
    ...
    @app_commands.describe(mode=constant.MODE, num_a="num_a", num_b="num_b")
    async def _random(
        self, interaction: discord.Interaction, mode: str, num_a: float, num_b: float
    ):
        return await interaction.response.send_message(
    ...
    async def _rand_autocomplete_mode(
        self, interaction: discord.Interaction, current: str
    ) -> list[app_commands.Choice[str]]:
        return [
            choice
