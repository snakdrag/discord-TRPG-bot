...
class ACT_MAIN(Cog_Extension):
    ...
    async def _dice(
        ...
        try:
            ...
            if not player:
                return await step.send(constant.ROLE + constant.NOT + constant.EXIST)
            attributes = (
                await step.find_one(constant.CARD, guild_id=step.guild_id) or {}
            ...
            if mode.startswith(Count_result.coc7e[:3]):
                try:
                    ...
                    return await step.send(constant.NOT + constant.NUM)
    ...
    async def _set(
        ...
        try:
            ...
            return await step.send(
                embed=await step.player_show(player_name, ID=player[constant.ID])
            )
        except AppError as e:
            return await step.send(e)

    @_set.autocomplete("mode")
    async def _set_autocomplete_mode(
