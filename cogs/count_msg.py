    ...
    async def _message(self, message: discord.Message):
        ...
        try:
            ...
            if command == r"\cut":
                ...
                try:
                    sum_val, num = int(parts[1]), int(parts[2])
                except ValueError:
                    return await message.channel.send(
                        f"{constant.NOT} {constant.INT} {constant.NUM}"
                    )
                if sum_val <= 0 or num <= 0:
                    return await message.channel.send(f"must > 0")
                if num == 1:
                    output = str(sum_val)
                else:
                    output = "+".join(map(str, Count_result.cut_result(sum_val, num)))
                return await message.channel.send(
                    f"{message.author.mention}\n`{sum_val} → {output}`"
                )
            if command == r"\int" or command == r"\float":
                ...
                try:
                    num_a, num_b = float(parts[1]), float(parts[2])
                except ValueError:
                    return await message.channel.send(f"{constant.NOT} {constant.NUM}")
