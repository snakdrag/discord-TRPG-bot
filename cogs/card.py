...
class CARD_MAIN(Cog_Extension):
                ...
                UpdateOne(
                    _FEATURE,
                    {
                        "$set": {
                            ...
                            constant.INCLUDE: (
                                int(include) if include.isdigit() else step.guild_id
                            ),
                        }
                    },
                    upsert=True,
            ...
            result = (
                await step.bulk_write(
                    UpdateOne(
                        ...
                        guild_id=step.guild_id,
                        query_override={
                            f"{constant.ATTRIBUTE}.{constant.NAME}": {"$ne": name}
                        },
                    )
                )
            )[_FEATURE]
            if not result.upserted_count:
    ...
    async def _attribute_change(
        ...
        try:
            ...
            result = (
                await step.bulk_write(
                    UpdateOne(
                        ...
                        {"$set": _query},
                        query_override={
                            f"{constant.ATTRIBUTE}.{constant.NAME}": target
                        },
                        guild_id=step.guild_id,
                    )
                )
            ...
            if result.modified_count == 0:
                return await step.send(f"{target} {constant.NOT}{constant.EXIST}")
            await step.bulk_write(
                UpdateMany(
                    constant.RACE,
                    {"$set": _query},
                    query_override={f"{constant.ATTRIBUTE}.{constant.NAME}": target},
                    guild_id=step.guild_id,
                )
            )
            return await step.send(
                f"{name or target} {constant.CHANGE}{constant.SUCCESS}"
            )
        except AppError as e:
            return await step.send(e)
        except Exception as e:
    ...
    async def _attribute_delete(
        ...
        try:
            ...
            if doc and doc.get(constant.GUILD_ID, step.guild_id) != step.guild_id:
                return await step.send(f"資料引入中，禁止使用")
            attributes: list[dict] = (await step.find_one(_FEATURE)).get(
                constant.ATTRIBUTE, []
            )
            idx = next(
                (i for i, a in enumerate(attributes) if a[constant.NAME] == target),
                None,
            )
            if idx is None:
                return await step.send(f"{target} {constant.NOT}{constant.EXIST}")
            await step.bulk_write(
                UpdateOne(
                    _FEATURE,
                    {"$pull": {constant.ATTRIBUTE: {constant.NAME: target}}},
                    guild_id=step.guild_id,
                ),
                UpdateMany(
                    constant.RACE,
            ...
            await step.db.bulk_write(
                *[
                    UpdateMany(
                        ...
                        {
                            "$unset": {
                                f"{constant.BASIC}.{idx}": 1,
                                f"{constant.MAX}.{idx}": 1,
                                f"{constant.NOW}.{idx}": 1,
                            }
                        },
                        guild_id=guild_id,
                    )
    ...
    async def _attribute_show(
        ...
        if not attributes:
            return await step.send(f"{target} {constant.NOT}{constant.EXIST}")
        attribute = next(
            (attr for attr in attributes if attr.get(constant.NAME) == target), {}
        )
    ...
    async def _show(
        ...
        step = Interaction(interaction)
        await step.first_step()
        embed = await step.card_show(
            guild_id=int(guild) if guild and guild.isdigit() else step.guild_id
        )
        return await step.send(embed=embed)


async def setup(bot: Bot):
    await bot.add_cogs(CARD_MAIN)
