...
class ROLE_CMD(Cog_Extension):
    ...
    async def _find_role_user(
        ...
        if not message.webhook_id:
            return await step.send(constant.NOT + constant.ROLE + constant.MESSAGE)
        message_data = await step.find_one(constant.MESSAGE, ID=message.id)
        if not message_data:
            return await step.send(constant.DATA + constant.NOT + constant.FIND)
        user_id = message_data[constant.USER_ID]
        user = await self.bot.get_target_user(user_id=user_id)
        if user:
            return await step.send(f"{user.mention}")
        else:
    ...
    async def _edit_role_message(
        ...
        if not message_data:
            return await step.send(constant.DATA + constant.NOT + constant.FIND)
        if interaction.user.id != message_data[constant.USER_ID]:
            return await step.send("你不是使用者")
    ...
    async def _delete_role_message(
        ...
        message_data = await step.find_one(constant.MESSAGE, ID=message.id)
        if not message_data:
            return await step.send(f"{constant.DATA}{constant.NOT}{constant.FIND}")
        if interaction.user.id != message_data[constant.USER_ID]:
            return await step.send("你不是使用者")
        try:
            ...
            return await step.send(
                constant.MESSAGE + constant.DELETE + constant.SUCCESS
            )
            ...
            try:
                await Message(self, message).webhook_delete()
            except discord.Forbidden:
                return await step.send("機器人權限不足")
            except Exception as e:
                raise e


async def setup(bot: Bot):
    await bot.add_cogs(ROLE_CMD)
