...
class BATTLE_MAIN(Cog_Extension):
    ...
    async def _join(
        ...
        try:
            player = await step.find_one(
                constant.PLAYER, name=role, user_id=step.user_id, guild_id=step.guild_id
            )
            ...
            result = (
                await step.bulk_write(
                    UpdateOne(
                        constant.PLAYER,
                        {"$set": {constant.GROUP: group}},
                        ID=player[constant.ID],
                    )
                )
            )[constant.PLAYER]
            if result.modified_count:
                return await step.send(constant.JOIN + group + constant.SUCCESS)
            else:
                raise AppError(constant.JOIN + group + constant.FAILED)
        except AppError as e:
            return await step.send(e)
    ...
    async def _kick(
        ...
        try:
            ...
            player = await step.find_one(
                ...
                name=player_name,
                user_id=int(user_id),
                guild_id=step.guild_id,
            )
            if not player:
                raise AppError(constant.PLAYER + constant.NOT + constant.EXIST)
            if not group:
                raise AppError(
                    f"{player_name} {constant.GROUP+constant.NOT+constant.EXIST}"
                )
            if player.get(constant.IS_TURN, 0):
                await self.next_turn(group, step.guild_id, player)
            result = (
                await step.bulk_write(
                    UpdateOne(
                        constant.PLAYER,
            ...
            if result.modified_count:
                return await step.send(constant.KICK + constant.SUCCESS)
            else:
                raise AppError(constant.KICK + constant.FAILED)
        except AppError as e:
            return await step.send(e)
        except Exception as e:
            raise e
    ...
    async def _leave(
        ...
        try:
            ...
            if result.modified_count:
                return await step.send(constant.LEAVE + constant.SUCCESS)
            else:
                raise AppError(constant.LEAVE + constant.FAILED)
        except AppError as e:
            return await step.send(e)
        except Exception as e:
            raise e
        ...
        try:
            ...
            result = (
                await step.bulk_write(
                    UpdateOne(
                        constant.CARD,
                        {"$set": {constant.BATTLE_ATTRIBUTE: attribute}},
                        ID=doc[constant.ID],
                    )
                )
            )[constant.CARD]
            if result.modified_count:
                return await step.send(
                    constant.ATTRIBUTE + constant.SET + constant.SUCCESS
                )
    ...
    async def _react(
        ...
        try:
            if item is None and skill is None:
                raise AppError(f"{constant.NOT+constant.ITEM}/{constant.SKILL}")
            player = await step.find_one(constant.PLAYER, name=role)
            if not player.get(constant.CAN_REACT, 0):
            ...
            items, skills = await asyncio.gather(
                step.get(constant.ITEM, role), step.get(constant.SKILL, role)
            )
            ...
            t_item: dict = (
                items[items_name.index(item)].get(constant.TARGET) if item else {}
            )
            t_skill: dict = (
                skills[skills_name.index(skill)].get(constant.TARGET) if skill else {}
            )
            t_item, t_skill = t_item or {}, t_skill or {}
            if int(t_item.get(constant.COST_TURN, 0)) * int(
                t_skill.get(constant.COST_TURN, 0)
            ):
                raise AppError(
            ...
            if not attacker:
                raise AppError(f"攻擊者 {constant.NOT} {constant.FIND}")
            ...
            await step.bulk_write(
                UpdateOne(
                    constant.PLAYER, {constant.CAN_REACT: 0}, ID=player[constant.ID]
                )
            )
            await step.send("立即結算")
            if item:
                await step.send(embed=await step.show(constant.ITEM, item))
            if skill:
                await step.send(embed=await step.show(constant.SKILL, skill))
    ...
    async def _act(
        ...
        try:
            if item is None and skill is None:
                raise AppError(f"{constant.NOT+constant.ITEM}/{constant.SKILL}")
            player = await step.find_one(constant.PLAYER, name=role)
            if not player.get(constant.IS_TURN, 0):
                raise AppError(f"{role} {constant.NOT+constant.CAN_REACT}")
            items, skills = await asyncio.gather(
                step.get(constant.ITEM, role), step.get(constant.SKILL, role)
            )
            ...
            cost_turn = int(t_item.get(constant.COST_TURN, 1)) + int(
                t_skill.get(constant.COST_TURN, 1)
            )
            group = player.get(constant.GROUP)
            cmd = f"{t_item.get(constant.PROACTIVE_EFFECT,"")};{t_skill.get(constant.PROACTIVE_EFFECT,"")}"
            target_group, target_name, target_id = target.split("--", 2)
            if target_group != group:
                raise AppError("不在同一場戰鬥")
            if t_item.get(constant.CAN_REACT) or t_skill.get(constant.CAN_REACT):
                await step.bulk_write(
                    UpdateMany(
                        constant.PLAYER,
                        {constant.CAN_REACT: True},
                        ID=player[constant.ID],
                    )
                )
            ...
            battle_data = {
                ...
                constant.CMD: cmd,
                constant.TARGET: {
                    constant.NAME: target_name,
                    constant.USER_ID: int(target_id),
                },
            ...
            if skill:
                await step.update_single_time(
                    constant.SKILL, role, skill, t_skill.get(constant.TIME)
                )
            if item:
                await step.update_single_time(constant.ITEM, role, item, -1)
            if item:
                await step.send(embed=await step.show(constant.ITEM, item))
            if skill:
                await step.send(embed=await step.show(constant.SKILL, skill))
            if (int(t_item.get(constant.CAN_REACT, 1)) - 1) * (
                int(t_skill.get(constant.CAN_REACT, 1)) - 1
            ):
    ...
    async def _skip(
        ...
        try:
            ...
            if can_react:
                await step.send("立刻結算")
                return await self._process_resolved(
            ...
            turn = await self.next_turn(
                player.get(constant.GROUP), step.guild_id, player
            )
            return await step.send(
                f"<@{turn[0][constant.USER_ID]}>:**{turn[0][constant.NAME]}**",
                embed=discord.Embed(
...
async def setup(bot: Bot):
    await bot.add_cogs(BATTLE_MAIN)
