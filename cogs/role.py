...
class ROLE_MAIN(Cog_Extension):
    ...
    async def _add(
        ...
        await step.db.bulk_write(
            UpdateOne(
                _FEATURE,
                {"$set": {constant.NAME: name, constant.CALL: call, constant.URL: url}},
                upsert=True,
                name=name,
    ...
    async def _change(
        ...
        player = await step.find_one(
            constant.PLAYER, name=target, user_id=step.user_id, guild_id=step.guild_id
        )
    ...
    @app_commands.describe(target=_FEATURE)
    @app_commands.autocomplete(
        target=_FEATURE_AUTOCOMPLETE,
    )
    async def _delete(
    ...
    @_group.command(
        name=constant.LIST, description=constant.SHOW + _FEATURE + constant.LIST
    )
    async def _list(
        self,
        interaction: discord.Interaction,
        ...
        return await step.send(
            f"{interaction.user.mention}: {roles_len}\n{"\n".join([
    ...
    async def _card_reset(
        ...
        embed = await step.player_show(
            role=target, user_id=step.user_id, guild_id=step.guild_id
        )
        return await step.send(embed=embed)
