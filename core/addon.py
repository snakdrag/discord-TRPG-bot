...
class Addon:
    ...
    async def send(
        ...
        *,
        ...
        if self._send_function:
            if embed is MISSING and content is MISSING:
                content = _const.NOT + _const.DATA
    ...
    async def bulk_write(
        ...
        *requests: UpdateOne | DeleteOne | UpdateMany | DeleteMany,
        ...
        for r in requests:
            if r.filter_params.get(_const.GUILD_ID) is None:
                r.filter_params[_const.GUILD_ID] = self.guild_id
            if r.filter_params.get(_const.USER_ID) is None:
                r.filter_params[_const.USER_ID] = self.user_id
    ...
    async def get(
        ...
        user_id = user_id or self.user_id
        guild_id = guild_id or self.guild_id
        using_id = (await self.find_one(_const.CARD, guild_id=guild_id)).get(
            _const.USING_ID, guild_id
        )
        return await self.db.find(
            feature,
            query_override={
                _const.GUILD_ID: using_id,
                f"{_const.DATA}.{user_id}.{role}": {"$exists": True},
            },
        )
    ...
    async def save(
        ...
        if data and data[0][_const.GUILD_ID] != self.guild_id:
            raise AppError(f"資料引入中，禁止使用")
        if name is not None:
        ...
        _quick_quary(
            _const.COST_TURN, int(cost_turn) if cost_turn is not None else None
        )
        _quick_quary(
            _const.CAN_REACT, int(can_react) if can_react is not None else None
        )
        _quick_quary(_const.TARGET_NUM, target_num)
        if not quary:
            raise AppError(_const.DATA + _const.NOT + _const.ENOUGH)
    ...
    async def give(
        ...
        if not doc:
            raise AppError(thing + _const.NOT + _const.EXIST)
        ...
        return (
            await self.bulk_write(
                UpdateOne(
                    feature,
                    {
                        "$inc": {f"{path}.{_const.NUM}": about_num},
                        "$set": {
                            f"{path}.{_const.TIME}": Count_result.dnd_result(
                                about_time
                            )[1],
                            f"{path}.{_const.ON}": on,
                        },
    ...
    async def remove(
        ...
        doc = await self.find_one(feature=feature, name=thing, guild_id=self.guild_id)
        current_data: dict = (
            doc.get(_const.DATA, {}).get(str(user_id), {}).get(role, {})
        )
        if current_data.get(_const.NUM, 0) <= 0:
            return (
                await self.bulk_write(
                    UpdateOne(
                        feature,
    ...
    async def delete(
        ...
        *,
        ...
        if doc[_const.GUILD_ID] != self.guild_id:
            raise AppError(f"資料引入中，禁止使用")
