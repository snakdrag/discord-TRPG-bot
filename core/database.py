...
class DataBase:
    def __init__(self, url: str):
        try:
            ...
            self._registry: dict[str, dict[str, _DB | list[str]]] = {
                ...
                _const.BATTLE: {
                    _DATA: self.battle,
                    _MODE: [_const.GUILD_ID, _const.GROUP],
                },
                _const.PLAYER: {
                    _DATA: self.player,
                    _MODE: [_const.GUILD_ID, _const.USER_ID, _const.NAME, _const.GROUP],
                },
                _const.SKILL: {
                    _DATA: self.skill,
                    _MODE: [_const.USING_ID, _const.NAME],
                },
                _const.STATE: {
                    _DATA: self.state,
                    _MODE: [_const.USING_ID, _const.NAME],
            ...
            print(f"{_const.DB}{_const.FAILED}:{e}")

    def close(self):
        if self.client:
            self.client.close()
            print(f"{_const.DB}{_const.CLOSE}")

    async def ping(self):
        if not self.client:
            return False
        try:
            await self.client.admin.command(command="ping")
            return True
        except Exception as e:
            print(e)
            return False
    ...
    async def find_one(
        ...
        *,
        ...
        if feature not in self._allow_db:
            raise AppError(f"{feature}{_const.NOT}{_const.ALLOW}")
        target_info: dict = self._registry[feature]
        collection: _DB = target_info[_DATA]
        if ID is not None:
            return await collection.find_one({_const.ID: ID}) or default
        query: dict = {}
        mapping: dict = {
            _const.NAME: name,
            _const.GROUP: group,
        ...
        using_id = (await self.card.find_one({_const.GUILD_ID: guild_id}) or {}).get(
            _const.USING_ID, guild_id
        )
        for field in target_info[_MODE]:
            val: Any = mapping.get(field)
            if val is not None:
                if field == _const.USING_ID:
                    query[_const.GUILD_ID] = using_id
                else:
                    query[field] = val
        if query_override:
        ...
        *requests: UpdateOne | DeleteOne | UpdateMany | DeleteMany,
        ...
        for req in requests:
            ...
            if target_id is not None:
                req.filter = {_const.ID: target_id}
            else:
                guild_id = params.get(_const.GUILD_ID)
                using_id = (
                    await self.card.find_one({_const.GUILD_ID: guild_id}) or {}
                ).get(_const.USING_ID, guild_id)
                for field in self._registry[feature][_MODE]:
                    val = params.get(field)
                    if val is not None:
                        if field == _const.USING_ID:
                            req.filter[_const.GUILD_ID] = using_id
                        else:
                            req.filter[field] = val
