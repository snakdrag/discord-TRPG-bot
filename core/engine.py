...
def _safe_eval_expr(node) -> int | float:
    ...
    if isinstance(node, _ast.UnaryOp):
        ...
        if op_type not in _ALLOWED_OPERATORS:
            raise AppError(f"不允許的運算符: {op_type.__name__}")
    ...
    if isinstance(data, list):
        if data and isinstance(data[0], dict):
            ...
            for d in data:
                res.update(d)
            return res
        if fields and len(data) == len(fields):
            return dict(zip(fields, data))
    return {}


def to_see_dict(data: dict):
...
class Count_result:
    ...
    def evaluate(expression: str):
        return round(
            _safe_eval_expr(
                _ast.parse(expression.strip().replace("^", "**"), mode="eval").body
            ),
            2,
        )
    ...
    def coc_result(difficulty: int, version: int = 7):
        ...
        if roll_value == 100:
            ...
            result = 0  # 大成功
        elif roll_value <= difficulty and version == 6:
            result = 4  # 成功
        elif roll_value > 95 and difficulty < 50:
            result = 6 if version == 6 else 5  # 大失敗/失敗
        elif roll_value <= difficulty / 5:
            result = 1  # 極限成功
        elif roll_value <= difficulty / 2:
            result = 2  # 困難成功
        elif roll_value <= difficulty:
            result = 3  # 通常成功
        else:
            result = 5  # 失敗
    ...
    def dnd_result(value: str | int | float):
        ...
        expression = value
        if value is None:
            return f"{_const.NOT} {_const.VALUE}", 1
        for m in reversed(list(_finditer(r"(\d+)[Dd](\d+)", value))):
            num, sides = int(m.group(1)), int(m.group(2))
            if num > 1000 or num < 0:
                raise AppError(f"不支援超過 1000 與低於 0")
            if sides > 90000000 or sides < 0:
                raise AppError(f"不支援超過 90000000 與低於 0")
    ...
    def list_result(options: list[str], mode: str = _const.CHOOSE):
        ...
        if mode == _const.CHOOSE:
            output = _random.choice(temp)
        elif mode == _const.RESORT:
            output = " ".join(temp)
        return output
    ...
    def rand_result(a: int | float, b: int | float, mode: str = _const.FLOAT):
        ...
        if mode == _const.INT:
            a, b, output = int(a), int(b), int(output)
...
class Engine(commands.Cog):
    ...
    async def next_turn(
        ...
        if battle_attribute not in base_field:
            raise AppError(battle_attribute + _const.NOT + _const.EXIST)
            ...
            next_idx = (
                next(
                    (
                        ...
                        and p[_const.NAME] == player[_const.NAME]
                    ),
                    -1,
                )
                + 1
