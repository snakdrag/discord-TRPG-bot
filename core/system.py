...
def get_folder_path(folder: str, file: str):
    return _join(_dirname(_abspath(file)), folder.replace(".", "/"))
...
async def load_folder(folder: str, folder_path: str, load_function):
    if _exists(folder_path):
        ...
        msg = f"{_const.FOLDER}:{folder_path}{_const.NOT}{_const.FIND}"
    return msg
...
class _DB:
    ...
    def init(self):
        ...
        return Delete(filter=self.filter, **self.kwargs)
