...
def replace_tags(text: str, stats_map: dict):
    for tag in _findall(r"\[(.+?)\]", text):
        if tag in stats_map:
            text = text.replace(f"[{tag}]", str(stats_map[tag]))
        else:
            raise AppError(f"{tag}{_const.NOT}{_const.EXIST}")
    return text
...
def get_stats_map(base_field: list, player: dict):
    ...
    for idx, field in enumerate(base_field):
        stats_map[f"B.{field}"] = (
            player[_BASIC][idx] if idx < len(player[_BASIC]) else 0
        )
...
class Command(Addon):
    ...
    def _get_stats_map(self):
        for idx, field in enumerate(self.base_field):
            self.stats_map[f"u.B.{field}"] = (
                self.u_player[_BASIC][idx] if idx < len(self.u_player[_BASIC]) else 0
            )
            self.stats_map[f"u.M.{field}"] = (
                self.u_player[_MAX][idx] if idx < len(self.u_player[_MAX]) else 0
            )
            self.stats_map[f"u.N.{field}"] = (
                self.u_player[_NOW][idx] if idx < len(self.u_player[_NOW]) else 0
            )
            self.stats_map[f"e.B.{field}"] = (
                self._player[_BASIC][idx] if idx < len(self._player[_BASIC]) else 0
            )
            self.stats_map[f"e.M.{field}"] = (
                self._player[_MAX][idx] if idx < len(self._player[_MAX]) else 0
            )
            self.stats_map[f"e.N.{field}"] = (
                self._player[_NOW][idx] if idx < len(self._player[_NOW]) else 0
            )
    ...
    async def execute(self):
        ...
        try:
            attributes: list[dict] = (
                await self.find_one(_const.CARD, guild_id=self.guild_id)
            ...
            targets = (
                await self.db.find(
                    _const.PLAYER,
                    group=self.u_player.get(_const.GROUP),
            ...
            queue = [("u", self.u_player)] + [
                ("e", p)
                for p in (
                    _sample(targets, self.target_num)
                    if len(targets) > self.target_num
                    else targets
                )
            ]
            for side, p in queue:
            ...
            if self.save_data:
                await self.bulk_write(
                    *[
                        UpdateOne(_const.PLAYER, {"$set": p}, ID=p[_const.ID])
                        for _, p in queue
                    ]
                )
    ...
    async def _apply_stat_change(self, file_key: str, raw_cmd: str):
        ...
        if attr not in self.f_idx:
            raise AppError(f"{attr} {_const.NOT} {_const.EXIST}")
        _idx, _key = self.f_idx[attr], _MODE_MAP[mode]
        old_val = self._player[_key][_idx]
        new_val = int(
            Count_result.dnd_result(
                str(self._player[_BASIC][_idx] if _key == _MAX else old_val)
                + await self._process_actions(raw_cmd)
            )[1]
        )
        if _key == _NOW:
    ...
    async def _process_actions(self, raw_cmd: str):
        ...
        for action_content in _get_actions(cmd_str):
            ...
            for act in parts[2:]:
                await self._handle_side_effect(act)
    ...
    async def _handle_side_effect(self, act: str):
        ...
        if func_name == _const.RETURN:
            for c in args:
                if (
                    Count_result.dnd_result(f"{self._roll_val}{c}")[1]
                    and self.save_data
                ):
                    self.return_text.append(f"{self._dice_expr}->{self._roll_val}{c}")
            return
        if not Count_result.dnd_result(f"{self._roll_val}{args[0]}")[1]:
            return
        if len(args) > 3:
            thing = str(args[3])
        else:
            ...
            if target_idx is None:
                raise AppError(f"{_const.DATA}{_const.NOT}{_const.EXIST}")
            thing = feature_list[target_idx].get(_const.NAME, None)
        if not thing:
            raise AppError(f"{_const.DATA}{_const.NOT}{_const.EXIST}")
        await (self.give if func_name == _const.GIVE else self.remove)(
            args[1],
            self._player[_const.NAME],
            thing,
            Count_result.dnd_result(args[2])[1],
            self._player[_const.USER_ID],
        )
