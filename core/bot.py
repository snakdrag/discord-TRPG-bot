...
class Bot(commands.Bot):
    ...
    async def setup_hook(self):
        ...
        self.managers = (
            list(app_info.team.members) if app_info.team else [app_info.owner]
        )
        print(await load_folder(self._folder, self._folder_path, self.load_extension))
        try:
            print(f"app_command.len:{len(await self.tree.sync())}")
        except Exception as e:
            print(f"app_command failed: {e}")
        ...
        async def on_app_command_error(
            ...
            if (
                isinstance(original_error, discord.errors.NotFound)
                and original_error.code == 10062
            ):
                return
            ...
            if guild:
                try:
                    guild_info = f"[{guild.name}]({(await interaction.channel.create_invite(
                    max_age=86400,max_uses=1,reason="Error reporting")).url})"
                except:
                    guild_info = guild.name
            else:
                guild_info = "私訊"
    ...
    async def notify_managers(self, content: str):
        ...
        for cont in contents:
            await asyncio.gather(
                *[user.send(cont) for user in self.managers], return_exceptions=True
            )
    ...
    def add_commands(
        self,
        *commands: app_commands.Command | app_commands.ContextMenu | app_commands.Group,
    ):
    ...
    async def load(self, name: str, *, package: str | None = None):
        try:
            ...
            await self.reload_extension(name, package=package)
        except Exception as e:
            raise e
