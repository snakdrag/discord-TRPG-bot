...
class Message(Addon):
    ...
    async def webhook_get(self):
        ...
        for webhook in webhooks:
            if webhook.user.id == me_id:
                return webhook
        self.webhook: discord.Webhook = await self.webhook_channel.create_webhook(
            name=WEBHOOK
        )
        return self.webhook

    async def webhook_send(self, content: str, call: str):
        role: dict = await self.db.role.find_one(
            {_const.CALL: call, _const.USER_ID: self.user_id}
        )
        if not role:
            return False
        ...
        message: discord.WebhookMessage = await self.webhook.send(
            ...
            thread=self.thread,
            wait=True,
            username=role.get(_const.NAME, UNKNOWN),
            avatar_url=role.get(_const.URL, MISSING),
        )
        await self.message.delete()
        await self.bulk_write(
            UpdateOne(
                _const.MESSAGE,
                {"$set": {_const.USER_ID: self.user_id}},
                upsert=True,
                ID=message.id,
...
class Interaction(Addon):
    ...
    async def first_step(self, *, ephemeral: bool = False, thinking: bool = False):
        self.ephemeral = ephemeral
        await self.interaction.response.defer(
            ephemeral=self.ephemeral, thinking=thinking
        )
    ...
    async def player_save(
        ...
        if race is None:
            ...
            attributes: list[dict] = (
                await self.find_one(_const.RACE, guild_id=self.guild_id, name=race)
            ).get(_const.ATTRIBUTE, [])
        base_value = [
            Count_result.dnd_result(attr.get(_const.VALUE, "0"))[1]
                ...
                UpdateOne(
                    _const.PLAYER,
                    {
                        "$set": {
                            _const.RACE: race,
                            _const.IS_TURN: 0,
                            _const.NOW: list(base_value),
                            _const.CAN_REACT: 0,
                            _const.GROUP: None,
                            _const.MAX: list(base_value),
                            _const.BASIC: list(base_value),
                        }
                    },
                    upsert=True,
                    name=role,
            ...
            *[
                UpdateMany(
                    ...
                    [
                        {
                            "$set": {
                                f"{_const.DATA}.{self.user_id}.{name}": f"${old_path}"
                            }
                        },
                        {"$unset": old_path},
                    ],
                    query_override={old_path: {"$exists": True}},
                    guild_id=self.guild_id,
                )
                for feature in [_const.SKILL, _const.STATE, _const.ITEM]
            ],
        )[_const.PLAYER]
        ...
        return (
            await self.bulk_write(
                ...
                *[
                    UpdateOne(
                        feature,
                        {"$unset": {f"{_const.DATA}.{self.user_id}.{role}": ""}},
                        guild_id=self.guild_id,
                    )
                    for feature in [_const.SKILL, _const.STATE, _const.ITEM]
                ],
    ...
    async def player_update(
        ...
        return await Command(
            self.db,
            ";".join(
                i.get(_const.PASSIVE_EFFECT, "")
                for i in _chain.from_iterable(
                    await asyncio.gather(
                        *[
                            self.get(f, role)
                            for f in [_const.SKILL, _const.STATE, _const.ITEM]
                        ]
                    )
                )
                if i.get(_const.DATA, {})
                .get(str(self.user_id), {})
                .get(role, {})
                .get(_const.ON, False)
            ),
            await self.find_one(_const.PLAYER, ID=ID, name=role),
        ).execute()
    ...
    async def player_show(
        ...
        player = await self.find_one(
            _const.PLAYER, ID=ID, name=role, user_id=user_id, guild_id=guild_id
        )
        if not player:
            return MISSING
        ...
        base_field = [
            attr[_const.NAME] for attr in attributes.get(_const.ATTRIBUTE, [])
        ]

        def _get_stats(key):
            return to_see_dict(to_dict(player.get(key, []), base_field))
    ...
    async def show(
        ...
        def quick_deal(place: str):
            ...
            if thing is not None:
                if place in [_const.COST_TURN, _const.CAN_REACT]:
                    thing = bool(thing)
            ...
            description="\n".join(
                [
                    ...
                    (
                        "\n".join(
                            [
                                f"• **{a[_const.NAME]}**: `{a[_const.VALUE]}`"
                                for a in attributes
                            ]
                        )
                        if attributes
    ...
    async def race_show(self, race: str):
        ...
        def _easy_to_see(key):
            return f"**{key}**: `{doc.get(key,UNKNOWN)}`"
    ...
    async def race_save(
        ...
        doc = await self.find_one(_const.CARD, guild_id=self.guild_id)
        if not doc:
            raise AppError(_const.DATA + _const.NOT + _const.EXIST)
        if doc.get(_const.GUILD_ID, self.guild_id) != self.guild_id:
            raise AppError(f"Now importing")
        if old_name is None:
            old_name = name
        elif name is None:
            name = old_name
        ...
        return await self.bulk_write(
            *[
                UpdateMany(
                    ...
                    query_override={f"{path}.{_const.TIME}": {"$exists": True}},
                    guild_id=self.guild_id,
                )
                for feature in [_const.SKILL, _const.STATE]
            ],
            UpdateMany(
                _const.STATE,
                {"$unset": {path: ""}},
                query_override={f"{path}.{_const.TIME}": {"$lte": 0}},
                guild_id=self.guild_id,
            ),
        )

    async def update_single_time(
        self, feature: str, role: str, thing: str, value: int, user_id: int = None
    ):
        user_id = user_id or self.user_id
        path = f"{_const.DATA}.{user_id}.{role}"
                ...
                [
                    {
                        "$set": {
                            f"{path}.{_const.TIME}": {
                                "$max": [0, {"$add": [f"${path}.{_const.TIME}", value]}]
                            }
                        }
                    }
                ],
